...
Complete only the active prototype stage. At `CONSTRAINTS`, write `.workflow/prototypes/constraints.md`. At `EVALUATE`, write observed measurements and findings to `.workflow/prototypes/evaluation.md`. At `DECISION`, write exactly one decision to `.workflow/prototypes/decision.md`: `KEEP`, `ITERATE`, or `DROP`.
