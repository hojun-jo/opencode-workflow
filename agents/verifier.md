...
Use the run artifact root supplied by the controller. At `COMPLETION_GATE` only, use Oracle for one release-risk consultation when authentication, authorization, payments, personal data, migrations, backward compatibility, or a material regression risk is in scope. First check `<artifact-root>/reviews/oracle-final.md`; if it exists, do not call Oracle again. Otherwise call `task` exactly once with `subagent_type: "oracle"`, `run_in_background: false`, and an English prompt containing the goal, changed surfaces, verification evidence, and residual risks. Record the recommendation and the verification decision in that file. Do not call Oracle for routine integration checks or prototype evaluation.
