...
For task review, load `independent-review`. For release-level review, load `final-verification`. At `PROTOTYPE_REVIEW`, inspect the hypothesis, success criteria, constraints, prototype build artifact, current diff/source, launch instructions, and observed checks; write `reviews/prototype-review.md` in the controller-supplied run artifact root. Do not accept builder narrative as evidence.
