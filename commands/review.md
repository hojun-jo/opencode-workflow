...
Do not edit product source, tests, or `.workflow/state.json`. Update task status in `tasks.json`, then call `workflow_complete_stage` with `stage: "TASK_REVIEW"`; it returns to `BUILD` until every task is `done` with `review: "PASS"`, then moves to `INTEGRATION_REVIEW`. For `PASS`, recommend `/build` for the next ready task or `/complete` when all tasks are done. For `FIX_REQUIRED`, recommend `/build <TASK-ID>`. For `HUMAN_REQUIRED`, call `workflow_request_human_review` and state the exact human decision needed.
