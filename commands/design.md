...
For an eligible high-impact product decision, the product-design agent may request one synchronous, read-only Sol Oracle consultation and record it in `<artifact-root>/design/oracle-consultation.md`, where `<artifact-root>` is `workflow.artifact_root` from `.workflow/state.json`. Never use Oracle for routine UX alternatives.
