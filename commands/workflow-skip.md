...
Do not skip a human gate, terminal stage, or a stage other than the current one. The controller dispatches the next non-human stage automatically. Report the `SKIPPED` stage, preserved reason, next stage, and any gate now waiting. If the reason is empty, stop and request one without changing state.
