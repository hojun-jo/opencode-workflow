...
Execute RED, GREEN, REFACTOR, VERIFY; record observed commands and outcomes. Keep scope to the task and relevant documentation. Mark the task `review_ready` in `tasks.json`, then call `workflow_complete_stage` with `stage: "BUILD"`; the controller sets `TASK_REVIEW`. Do not self-review, directly edit `.workflow/state.json`, or mark the task done. Recommend `/review <TASK-ID>`.
