...
Use the OMO Oracle for one read-only Sol consultation only when an architecture decision is hard to reverse, cross-cutting, security-sensitive, or costly to correct. Record it in `<artifact-root>/architecture/oracle-consultation.md`, where `<artifact-root>` is `workflow.artifact_root` from `.workflow/state.json`; never use it for ordinary task decomposition. Do not edit product source, tests, or `.workflow/state.json`. Complete only the active planning stage; the controller dispatches each subsequent planning stage automatically and stops at `IMPLEMENTATION_REVIEW`. Recommend `/approve-plan` or `/plan <requested changes>`.
